...
def findWeeklyCross(s_dic):
    ...
    for db_name in ['tsxci', 'nasdaq100', 'sp100', 'eei']:
        ...
        for ticker in indexes['symbol'].tolist():
            # if ticker == "SAP":  # Only process SAP (adjust as needed)
            df = pd.read_sql(
...
def check_macd_conditions(df):
    ...
    # Condition 2: MACD crossed above Signal Line
    condition_2 = (
            df_weekly['MACD'].iloc[-2] < df_weekly['Signal'].iloc[-2] and
