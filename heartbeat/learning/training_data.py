...
from ..models import Quote, Report, Tdata, Findex, Gaps
...
def ema(df, span):
    ...
    return df['close'].ewm(span=span, adjust=False).mean()[-378:]
...
def get_report(s, tdata, db_name):
    ...
    oneyearago =  max_date - timedelta(days=365)
    oneyearago = oneyearago.to_pydatetime().strftime("%Y-%m-%d")
    max_date = max_date.to_pydatetime().strftime("%Y-%m-%d")
    df = df.loc[oneyearago:max_date]
    ...
    df.drop(df.index[df['yr_high'] == 0] & df.index[df['yr_low'] == 0] &
            ...
            df.index[df['high_volume'] == 0]  & df.index[df['rsi'] == '0'] &
            df.index[df['macd'] == '0'] & df.index[df['bolling'] == '0'] , inplace=True)
    # # Copy symbol column
    # if db_name == 'tsxci':
...
def add_gaps(s_l, report, db_name):
    # db_name = 'tsxci' # CHECKPOINT
    gaps = pd.read_sql(s_l.query(Gaps).filter(Gaps.index == db_name).statement, s_l.bind, index_col='date').drop(columns=['id'])
    gaps['gap'] = True
    gaps = gaps[['symbol','gap']]
    gaps.reset_index(inplace=True)
    report = pd.merge(report, gaps, how='left', on=['date','symbol'])
    report['gap'].replace(np.nan, False, inplace=True)
    return report
...
def fact_check(s, df):
    for index, row in df.iterrows():
        ...
        try:
            ...
            fprice_ema5 = slice_datetime(rema5, rdate, 5, 'close') # price at 5 days moving forward from recorded ema5
            fprice_ema21 = slice_datetime(rema21, rdate, 21, 'close') # price at 21 days moving forward from recorded ema5
            # Delta prices
    ...
    print('Total checked %s rows.' % str(len(df)))
    return df
